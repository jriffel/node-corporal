...
var loop = module.exports.loop = function(options, onCommand) {
    if (typeof options === 'function' && !onCommand) {
        onCommand = options;
        options = null;
    }
...
function _resetReadLine(state) {
    ...
    state.rl = readline.createInterface({
        ...
        'completer': function(line, callback) {
            if (typeof state.autocomplete !== 'function') {
...
function _bindSigint(state) {
    state.rl.once('SIGINT', function() {
        ...
        state.output.write('\n');
        return _sendResult(state, Object.assign(new Error('User pressed CTRL+C'), {'code': 'SIGINT'}));
    });
}
...
function _sendResult(state, err, args, str) {
    ...
    var finalArgs = _getFinalArguments(args);
    return state.callback(err, finalArgs ? finalArgs.map(function(a) { return a.str; }) : finalArgs, str);
}
